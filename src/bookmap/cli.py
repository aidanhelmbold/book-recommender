...
def bridges(
    ...
    with _open_readable(db) as store:
        projection = project(store)
        try:
            payload = find_bridges(
                store,
                projection,
                seeds=_split_seeds(seeds) if seeds else None,
                top_n=top_n,
            )
    ...
    if not payload["bridges"]:
        ...
        if payload["seed_communities"]:
            console.print("[dim]try without --seeds, or seed books from more genres[/dim]")
        return

    if payload["unresolved"]:
        console.print(
            f"[yellow]could not find:[/yellow] "
            f"{markup_escape(', '.join(payload['unresolved']))}"
        )
